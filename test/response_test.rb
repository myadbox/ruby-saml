...
class RubySamlTest < Test::Unit::TestCase
  ...
  context "Response" do
    should "raise an exception when response is initialized with nil" do
      assert_raises(ArgumentError) { James::RubySaml::Response.new(nil) }
    end

    should "be able to parse a document which contains ampersands" do
      JamesXmlSecurity::SignedDocument.any_instance.stubs(:digests_match?).returns(true)
      James::RubySaml::Response.any_instance.stubs(:validate_conditions).returns(true)

      response = James::RubySaml::Response.new(ampersands_response)
    ...
    should "adapt namespace" do
      response = James::RubySaml::Response.new(response_document)
      assert_not_nil response.name_id
      response = James::RubySaml::Response.new(response_document_2)
      assert_not_nil response.name_id
      response = James::RubySaml::Response.new(response_document_3)
      assert_not_nil response.name_id
    end

    should "default to raw input when a response is not Base64 encoded" do
      decoded  = Base64.decode64(response_document_2)
      response = James::RubySaml::Response.new(decoded)
      assert response.document
    end

    context "Assertion" do
      should "only retreive an assertion with an ID that matches the signature's reference URI" do
        response = James::RubySaml::Response.new(wrapped_response_2)
    ...
    context "#is_valid?" do
      should "return false when response is initialized with blank data" do
        response = James::RubySaml::Response.new('')
      ...
      should "return true when the response is initialized with valid data" do
        response = James::RubySaml::Response.new(response_document_4)
        response.stubs(:conditions).returns(nil)
        assert !response.is_valid?
        settings = James::RubySaml::Settings.new
        assert !response.is_valid?
      ...
      should "support dynamic namespace resolution on signature elements" do
        response = James::RubySaml::Response.new(fixture("no_signature_ns.xml"))
        response.stubs(:conditions).returns(nil)
        settings = James::RubySaml::Settings.new
        response.settings = settings
        settings.idp_cert_fingerprint = "28:74:9B:E8:1F:E8:10:9C:A8:7C:A9:C3:E3:C5:01:6C:92:1C:B4:BA"
        JamesXmlSecurity::SignedDocument.any_instance.expects(:validate_signature).returns(true)
        assert response.validate!
      end

      should "validate ADFS assertions" do
        response = James::RubySaml::Response.new(fixture(:adfs_response_sha256))
        response.stubs(:conditions).returns(nil)
        settings = James::RubySaml::Settings.new
        settings.idp_cert_fingerprint = "28:74:9B:E8:1F:E8:10:9C:A8:7C:A9:C3:E3:C5:01:6C:92:1C:B4:BA"
        response.settings = settings
      ...
      should "validate the digest" do
        response = James::RubySaml::Response.new(r1_response_document_6)
        response.stubs(:conditions).returns(nil)
        settings = James::RubySaml::Settings.new
      ...
      should "validate SAML 2.0 XML structure" do
        ...
        response = James::RubySaml::Response.new(Base64.encode64(resp_xml))
        response.stubs(:conditions).returns(nil)
        settings = James::RubySaml::Settings.new
        settings.idp_cert_fingerprint = signature_fingerprint_1
        response.settings = settings
        assert_raises(James::RubySaml::ValidationError, 'Digest mismatch'){ response.validate! }
      end
    end
    ...
    context "#name_id" do
      ...
      should "be extractable from an OpenSAML response" do
        response = James::RubySaml::Response.new(fixture(:open_saml))
        assert_equal "someone@example.org", response.name_id
      end

      should "be extractable from a Simple SAML PHP response" do
        response = James::RubySaml::Response.new(fixture(:simple_saml_php))
        assert_equal "someone@example.com", response.name_id
      end
    end
    ...
    context "#check_conditions" do
      should "check time conditions" do
        ...
        response = James::RubySaml::Response.new(response_document_6)
        assert response.send(:validate_conditions, true)
        time     = Time.parse("2011-06-14T18:25:01.516Z")
        Time.stubs(:now).returns(time)
        response = James::RubySaml::Response.new(response_document_5)
        assert response.send(:validate_conditions, true)
      end
      ...
      should "optionally allow for clock drift" do
        ...
        response = James::RubySaml::Response.new(response_document_5, :allowed_clock_drift => 0.515)
        assert !response.send(:validate_conditions, true)

        Time.stubs(:now).returns(Time.parse("2011-06-14T18:21:01Z"))
        response = James::RubySaml::Response.new(response_document_5, :allowed_clock_drift => 0.516)
        assert response.send(:validate_conditions, true)
      end
    end
    ...
    context "#attributes" do
      ...
      should "not raise errors about nil/empty attributes for EncryptedAttributes" do
        response = James::RubySaml::Response.new(response_document_7)
        assert_equal 'Demo', response.attributes["first_name"]
      end

      should "not raise on responses without attributes" do
        response = James::RubySaml::Response.new(response_document_4)
        assert_equal James::RubySaml::Attributes.new, response.attributes
      end

      context "#multiple values" do
        should "extract single value as string" do
          response = James::RubySaml::Response.new(fixture(:response_with_multiple_attribute_values))
          assert_equal "demo", response.attributes[:uid]
        end

        should "extract single value as string in compatibility mode off" do
          response = James::RubySaml::Response.new(fixture(:response_with_multiple_attribute_values))
          James::RubySaml::Attributes.single_value_compatibility = false
          assert_equal ["demo"], response.attributes[:uid]
          # classes are not reloaded between tests so restore default
          James::RubySaml::Attributes.single_value_compatibility = true
        end
